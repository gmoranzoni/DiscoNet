...
void mcode_complex(int* neighbor,int* neighbor_indx,float* vertex_weight,float* D,int* seed_vertex,int* seen,int* COMPLEX)
{
    int seed,i;
    seed=*seed_vertex;
    if(!seen[seed]){
        ...
        if((neighbor_indx[seed+1]-neighbor_indx[seed])>0){
            ...
            for(i=neighbor_indx[seed];i<neighbor_indx[seed+1];i++){
                if(vertex_weight[neighbor[i]] >= (vertex_weight[seed] * (1-*D))){
                  mcode_complex(neighbor,neighbor_indx,vertex_weight,D,&(neighbor[i]),seen,COMPLEX);
                }
            }
        }
    }
